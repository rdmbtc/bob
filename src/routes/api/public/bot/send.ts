    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          // 3. Resolve sender Circle Wallet ID via listWallets
          ...
          try {
            ...
            if (!resolvedId) throw new Error(`No Circle wallet found for address ${senderReg.wallet_address}`);
          ...
          const { data: txRow, error: insertError } = await supabase
            .from("transactions")
            .insert({ twitter_handle: recipient, amount_usdc, status: "pending" })
            .select("id")
            .maybeSingle();
          ...
          // 5. Execute USDC transfer from sender's Circle wallet
          let txHash: string;
          try {
            ...
            await supabase.from("transactions").update({ status: "failed" }).eq("id", txRow.id);
            return new Response(
          ...
          // 6. Mark confirmed
          await supabase.from("transactions").update({ tx_hash: txHash, status: "confirmed" }).eq("id", txRow.id);
